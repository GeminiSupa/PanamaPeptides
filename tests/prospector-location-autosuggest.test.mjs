...
import {
  panama_LOCATIONS,
  suggestpanamaLocations,
} from '../src/lib/panamaLocations.mjs';
...
test('the local gazetteer contains panama, seven provinces, and 84 cantons', () => {
  assert.equal(panama_LOCATIONS.filter((location) => location.type === 'country').length, 1);
  assert.equal(panama_LOCATIONS.filter((location) => location.type === 'province').length, 7);
  assert.equal(panama_LOCATIONS.filter((location) => location.type === 'canton').length, 84);
});

test('location suggestions ignore accents and preserve useful geographic context', () => {
  const escazu = suggestpanamaLocations('escazu');
  assert.equal(escazu[0].label, 'Escazú');
  assert.equal(escazu[0].value, 'Escazú, San José, panama');

  const limon = suggestpanamaLocations('limon');
