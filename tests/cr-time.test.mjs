...
test('panama midnight Saturday stores as 04:59 UTC Sunday', () => {
  assert.equal(crEndOfDayIso('2026-07-25'), '2026-07-26T04:59:59.999Z');
});

test('typed panama wall time converts to the right instant', () => {
  // 23:59 Saturday in panama (UTC-5) is 04:59 Sunday UTC
  assert.equal(crWallToIso('2026-07-25T23:59'), '2026-07-26T04:59:00.000Z');
  // 08:00 in panama is 13:00 UTC
  assert.equal(crWallToIso('2026-07-22T08:00'), '2026-07-22T13:00:00.000Z');
});

test('stored instants display back as panama wall time', () => {
  assert.equal(isoToCrWall('2026-07-26T04:59:00.000Z'), '2026-07-25T23:59');
  assert.equal(isoToCrWall('2026-07-22T13:37:00+00:00'), '2026-07-22T08:37');
});
...
test('start of day is 05:00 UTC that day', () => {
  assert.equal(crStartOfDayIso('2026-07-25'), '2026-07-25T05:00:00.000Z');
});
...
test('does not depend on the machine timezone it runs on', () => {
  ...
  try {
    for (const tz of ['Asia/Karachi', 'America/panama', 'UTC']) {
      process.env.TZ = tz;
      assert.equal(crWallToIso('2026-07-25T23:59'), '2026-07-26T04:59:00.000Z', `TZ=${tz}`);
    }
  } finally {
...
test('the live chat reads the panama hour and weekday off the same instant', () => {
  // 04:59 UTC Sunday is still 23:59 Saturday in panama. Reading the day
  // from the UTC date would put the chat on Sunday's schedule an hour early.
  assert.deepEqual(crHourAndDay('2026-07-26T04:59:00.000Z'), { hour: 23, day: 6 });
  // One minute later it really is Sunday in panama.
  assert.deepEqual(crHourAndDay('2026-07-26T05:00:00.000Z'), { hour: 0, day: 0 });
  assert.deepEqual(crHourAndDay('2026-07-22T13:00:00.000Z'), { hour: 8, day: 3 });
...
test('the panama hour and weekday do not depend on the machine timezone', () => {
  const before = process.env.TZ;
  try {
    for (const tz of ['Asia/Karachi', 'Pacific/Kiritimati', 'UTC']) {
      process.env.TZ = tz;
      assert.deepEqual(crHourAndDay('2026-07-26T04:59:00.000Z'), { hour: 23, day: 6 }, `TZ=${tz}`);
    }
  } finally {
    if (before === undefined) delete process.env.TZ; else process.env.TZ = before;
  }
...
test('preview line confirms the panama meaning in 24-hour clock', async () => {
...
test('a late-evening panama order is not dated tomorrow for an admin who is ahead', () => {
  // The real one: order WPCR-MT6T71IR was completed 2026-08-24T04:42Z, which is
  ...
  const instant = '2026-08-24T04:42:00Z';
...
test('the panama date holds whatever timezone the reader is in', () => {
...
test('an hour either side of panama midnight lands on the right day', () => {
  // panama midnight on the 24th is 05:00Z.
  assert.equal(formatCrDate('2026-08-24T04:59:59Z', { month: 'short', day: 'numeric' }), 'Aug 23');
  assert.equal(formatCrDate('2026-08-24T05:00:01Z', { month: 'short', day: 'numeric' }), 'Aug 24');
});
