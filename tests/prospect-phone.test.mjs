...
test('a national number is completed with the country dial code', () => {
  // tel:88887777 is how panaman sites publish a number. Sent to wa.me as
  // published it reads as country code +888, which belongs to nobody.
  assert.equal(whatsappDialableNumber('88887777', 'panama'), '50688887777');
...
test('a number that already carries its country code is left alone', () => {
  assert.equal(whatsappDialableNumber('+506 8888 7777', 'panama'), '50688887777');
  assert.equal(whatsappDialableNumber('50688887777', 'panama'), '50688887777');
});

test('a foreign number on a local business page keeps its own country code', () => {
  assert.equal(whatsappDialableNumber('+1 212 555 1234', 'panama'), '12125551234');
});

test('a number that cannot be dialed anywhere is refused', () => {
  // Ten digits, no country code, and not the national length for panama.
  assert.equal(whatsappDialableNumber('2222333444', 'panama'), null);
  assert.equal(whatsappDialableNumber('123', 'panama'), null);
});
...
test('country names resolve in either language, and by ISO code', () => {
  assert.equal(prospectDialPlan('panama')?.dial, '506');
...
test('permission cannot be verified against an undialable number', () => {
  assert.equal(hasUsableProspectWhatsAppIdentity({ country: 'panama', phone: '88887777' }), true);
  assert.equal(hasUsableProspectWhatsAppIdentity({ country: 'panama', phone: '2222333444' }), false);
});
