/**
 * Reads a business's own website for the contact details it publishes.
 *
 * Lifted out of the admin route that used to hold it so a background worker can
 * run the same scan. Nothing about the crawl changed in the move: the SSRF
 * guard that refuses private addresses, the redirect ceiling, the byte cap and
 * the crawl budget are all as they were, and are the reason this must never be
 * pointed at a URL a stranger supplied.
 *
 * What it returns is deliberately more than contacts. `emailPermissionSourceUrl`
 * is the page a mailto: was actually published on, which is the evidence the
 * outreach send gate demands before it will send anything — so a prospect
 * enriched here becomes contactable as a side effect of being scanned, with no
...
import {
  ...
  rankEmails,
} from './prospectWebEnrichment.mjs';
import { normalizeProspectPeople, upgradeContactPermission } from './prospects.mjs';
...
export async function enrichFromWebsite({ websiteUrl: rawWebsiteUrl, organizationName = '' } = {}) {

  const websiteUrl = String(rawWebsiteUrl || '').trim().slice(0, 1000);
  if (!websiteUrl) throw new Error('A business website is required');
  ...
  try {
    peopleResult = await extractPeopleWithGemini({
      ...
      phones,
      linkedinUrls,
      companyName: String(organizationName || '').trim().slice(0, 240),
      domainChecks,
    });
  } catch (error) {
...
/**
 * A scan result folded into the columns of the prospect it came from.
 *
 * Every field falls back to what the prospect already holds, so a scan that
 * finds nothing new can never blank something a person entered by hand — and a
 * permission is only ever raised, never lowered, because upgradeContactPermission
 * keeps the stronger of the two. The whole thing is written once here so the
 * admin tab and the background worker cannot drift into applying the same scan
 * two different ways.
 */
export function enrichmentUpdates(payload = {}, target = {}) {
  ...
  const emailRaised = emailPermissionStatus !== (target.email_permission_status || 'unknown');
  const whatsappRaised = whatsappPermissionStatus !== (target.whatsapp_permission_status || 'unknown');
  ...
  return {
    ...
    email_permission_basis: emailRaised ? 'published_business_contact' : target.email_permission_basis || null,
    email_permission_source_url: emailRaised ? payload.emailPermissionSourceUrl : target.email_permission_source_url || null,
    email_permission_evidence: emailRaised ? payload.emailPermissionEvidence : target.email_permission_evidence || null,
    whatsapp_permission_status: whatsappPermissionStatus,
    whatsapp_permission_basis: whatsappRaised ? 'published_business_contact' : target.whatsapp_permission_basis || null,
